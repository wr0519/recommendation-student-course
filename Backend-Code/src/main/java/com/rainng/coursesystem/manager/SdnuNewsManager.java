...
public class SdnuNewsManager extends BaseManager {
    ...
    private static final String CRAWL_TARGET_URL = "http://www.tyut.edu.cn/xyxw/lgyw.htm";
    private static final String BASE_URL = "http://www.tyut.edu.cn/";
    ...
    private List<SdnuNewsBO> parseNews(Document pageDoc) {
        Elements elements = pageDoc.body()
                .getElementsByClass("ul-list-d5 fix Uani fadeInUp").get(0)
                .getElementsByTag("li");
        ...
        for (Element element : elements) {
            ...
            Element aTag = element.getElementsByTag("h3").get(0);
            Element dateTag = element.getElementsByClass("date").get(0);
            String url = BASE_URL + element.getElementsByTag("a").get(1).attr("href");
