...
public class CourseSelectService extends BaseService {
    ...
    public ResultVO getCourseDeatil(Integer courseId){
        return result(manager.getCourseDetail(courseId));
    }

    public ResultVO getRecommend() {//获取推荐课程列表
        Integer studentId = getUserId();//从Session中获取用户ID
        List<StudentCourseSelectItemBO> boList = manager.getRecommend(studentId);//调用manager层代码与数据库进行交互
        List<StudentCourseSelectItemVO> voList = new ArrayList<>(boList.size());//推荐课程列表
        //对数据库查回的返回值进行遍历，存储到Volist中
    ...
    public boolean delrecommend(Integer courseId){
        Integer studentId = getUserId();
        return manager.delRecommend(studentId,courseId);
    }
